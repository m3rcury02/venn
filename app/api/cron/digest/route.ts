...
// Group names and display names are free text (SPEC §11: "free-text list names
// and notes are where abuse appears"), and both are interpolated straight into
// this route's email HTML. Escape before every interpolation site.
...
export async function GET(request: Request) {
  ...
  // Fetch all users from auth admin API. listUsers() defaults to 50 per page
  // and does not return everyone in one call -- loop until a short page ends
  // it, or every recipient past the first 50 silently never gets a digest.
  type AuthUser = NonNullable<
    Awaited<ReturnType<typeof service.auth.admin.listUsers>>["data"]
  >["users"][number];
  ...
  for (const user of allUsers) {
    ...
    if (memberError) {
      // Loud on purpose: a missing service_role grant on group_members would
      // otherwise degrade to "no nights" for every recipient with no error
      // anywhere in the response.
      console.error("digest: group_members query failed", memberError.message);
    }

    let loggedNights: { movieTitle: string; groupName: string }[] = [];
    if (memberRows && memberRows.length > 0) {
      const groupIds = memberRows.map((m) => m.group_id);
      const { data: rawNights, error: nightsError } = await service
        .from("movie_nights")
      ...
      if (nightsError) {
        console.error("digest: movie_nights query failed", nightsError.message);
      }
    ...
    const html = `
      <div style="font-family: system-ui, sans-serif; background-color: #0b0b0c; color: #f0f0f2; padding: 32px; max-width: 600px; margin: 0 auto; border-radius: 8px;">
          ...
          friendAdditions.length > 0
            ? `<div style="margin-bottom: 24px;">
                ...
                <ul style="padding-left: 20px; color: #d0d0d5;">
                  ${friendAdditions.map((item) => `<li style="margin-bottom: 6px;"><strong>${escapeHtml(item.addedBy)}</strong> added <strong>${escapeHtml(item.title)}</strong></li>`).join("")}
                </ul>
              </div>`
            : ""
        }
        ...
        ${
          loggedNights.length > 0
            ? `<div style="margin-bottom: 24px;">
                ...
                <ul style="padding-left: 20px; color: #d0d0d5;">
                  ${loggedNights.map((night) => `<li style="margin-bottom: 6px;"><strong>${escapeHtml(night.groupName)}</strong> logged <strong>${escapeHtml(night.movieTitle)}</strong></li>`).join("")}
                </ul>
              </div>`
            : ""
        }
        ...
        <div style="border-top: 1px solid #202024; padding-top: 16px; margin-top: 32px; font-size: 12px; color: #6e6e73;">
          ...
          <p><a href="${escapeHtml(baseUrl)}/settings/notifications" style="color: #4a9eff; text-decoration: underline;">Manage notification preferences</a></p>
        </div>
      </div>
    `;
