...
export default async function ExplorePage() {
  ...
  // Checked separately from an empty feed so the page can say "slow down"
  // rather than "Nothing left to rate", which would be false.
  const limited =
    typeof userId === "string" && !(await withinRateLimit(supabase, "feed"));
  const cards =
    typeof userId === "string" && !limited
      ? await exploreFeed(userId, region, 1)
      : [];
      ...
      <main className="flex flex-1 flex-col">
        {cards.length > 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-5 py-20 text-center">
            <VennMark size={44} />
            <p className="t-section text-3xl text-fg">
              {limited ? "Slow down a moment" : "Nothing left to rate"}
            </p>
            <p className="t-body mx-auto max-w-sm text-[15px] text-fg-dim">
              {limited
                ? "You've loaded a lot of trailers in the last minute. Try again shortly."
